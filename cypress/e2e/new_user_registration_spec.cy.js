...
describe("Acccess the oncology platform",()=>{
    beforeEach(()=>{
      ...
      it.only("test1-tests fucntionality of the registration module",()=>{
        ...
        cy.get("#idValue").type("31459014{enter}")
        cy.get('input[name="nationalIdNumber"]').type("31459014").should("be.visible")
        cy.get('input[name="other-identifiers"]').check()
        cy.get('input[name="personName.familyName"]').type("Test")
        cy.get('input[name="personName.givenName"]').type("Jake")
        cy.get('input[name="personName.middleName"]').type("Patient")
        cy.get('input[value="M"]').check()
        cy.get('#patient-birthdate_date').type("10-Dec-1996")
        ...
        cy.get("input[name='telephoneContact']").type("0790181478")
